...
function setupEvents() {
  ...
  function renderShowHero(hero, details, showName, stats) {
    const poster = purl(
      details.poster || details.image || details.poster_path ||
      details.images?.poster || details.all_images?.poster ||
      details.thumb || details.thumbnail || details.cover ||
      details.artwork || details.logo || ""
    );
    const fanart = purl(
    ...
    <div class="simple-hero">
       ...
       <div class="simple-hero-info">
         ...
         <div class="simple-meta-row">
           ...
           ${network ? `<span class="simple-stat-badge">${esc(network)}</span>` : ""}
  ...
  `;
    ...
    const searchInput = body.querySelector(".season-search-input");
    if (searchInput) {
      ...
      // Focus if we were already searching (only if it matches the current focus potentially)
      // but simpler to just keep state.
    }
    ...
    body.querySelectorAll(".season-quick-btn").forEach(btn =>
      btn.addEventListener("click", () => {
  ...
  function seasonEpisodeRowHTML(ep, showId, seasonNumber) {
    ...
    <div class="ep-row ${ep.watched ? "watched" : ""}">
      <div class="ep-thumb ${ep.thumbnail ? "" : "ep-thumb--ghost"}">
        ${ep.thumbnail
        ? `<img src="${ep.thumbnail}" alt="" data-fallback="thumb-fallback">`
